...
const fetchPosts = async (query?: string, page: number = 1): Promise<Post[]> => {
  const offset = (page - 1) * PAGE_SIZE;
  const { data } = await api.get('/posts/', {
    params: { q: query, offset, limit: PAGE_SIZE },
  })
  return data
}
...
function PostsComponent() {
  const { posts, total }: { posts: Post[]; total: number } = useLoaderData({ from: '/posts' });
  ...
  const handleSearch = (query: string) => {
    navigate({
      search: { q: query.trim() || undefined, page: 1 }, // Use undefined to remove the param
    });
  };
            ...
            {totalPages > 1 && (
              <div className="mt-6 flex items-center justify-between border-t pt-4">
                <Link
                  ...
                  disabled={page <= 1}
                  className="inline-block rounded-md bg-cyan-600 px-5 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-cyan-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600 aria-disabled:cursor-not-allowed aria-disabled:opacity-50"
                >
                  Prev
                </Link>
